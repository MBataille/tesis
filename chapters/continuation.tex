...
usually available for the problems studied in this work, we must resort / are required to numerical methods.
...
[It will be assumed ...
+ formalidad uso de that, we will assume that we previously know a solution...
Borrar recall newton's method...]
...
to the Jacobian in Newton's method, the stability of the solution can be tracked and bifurcation points can be detected by tracking changes in the sign of the determinant of the Jacobian.
...
the previous solution $\bm{u_0}$ as initial guess for Newton's method. Finally, is repeated the process
...
\begin{exmp}
   ...
   the stable and unstable equilibria of Eq.~(\ref{eq:pre_nc_she}) will be computed.     
    ...
    Fig ... illustrates / shows the case where $\varepsilon = 0.1$, for which the system is bistable. Indeed, if we start the algorithm
...
Note that the natural continuation succeeds at finding the lower and upper branch of the solution
in this example.
...
Although in this case, it is not difficult to find this guess, for a higher-dimensional system where the
...
of the continuation method, the previous tangent is unkown. In that case, we can choose the
...
is positive to move forward or negative to move backward.
...
\begin{exmp}
    In order to illustrate, it is useful to revisit the previous example and implement the pseudo-arclength continuation
